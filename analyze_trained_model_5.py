...
# NOTE: all four action primitives are enabled explicitly in the DebugSimEnv
# constructor below (section 4), so no override of args.action_primitives
# is needed here.

# ============================================================
# 0. OUTPUT DIR (fresh run each time — previous socket_eval is wiped)
# ============================================================
...
class DebugSimEnv(SimEnv):
    ...
    def dump_current_video(self, step_id):
        # Not called anywhere in the main pipeline right now — kept as a
        # manual/console helper for dumping the current video buffer on demand.
        import cv2
    ...
    def step(self, value_maps):
        ...
        # Image-identity/checksum debug from tracking down stale image-buffer
        # bugs — no longer needed now the save path is verified working.
        # print("\n===== SAVE DEBUG =====")
        # print("STEP:", step_id)
    ...
    def get_max_value_valid_action(self, value_maps):
        ...
        if action is not None:
            ...
            # Cloth bounding-box debug (from depth threshold) — comment back
            # in if cloth extents ever need checking again.
            ...
            # Image-identity/checksum debug, matching the AFTER-frame block
            # in step() — no longer needed now the save path is verified.
            # print("\n===== SAVE DEBUG =====")
            # print("STEP:", step_id)
            ...
            # print("IMAGE SHAPE:", img.shape)
            # print("IMAGE SUM:", img.sum())
            # print("BEFORE FILE:", f"socket_eval/step_{step_id:03d}_01_before.png")
            ...
            # Rotation/distance debug from the coordinate-mismatch debugging
            # phase — vec/angle/grasp_dist_px only ever fed these prints,
            # nothing downstream uses them. Uncomment if grasp point drift
            ...
            # ---------------------------------------------
            # RENDER_DIM -> ROBOT CAMERA SPACE SCALE
            # camera_scale is currently hardcoded/derived, not physically
            # calibrated — deferred pending colleague consultation.
            # ---------------------------------------------
            CAMERA_TARGET = 720
            ...
            # ---------------------------------------------
            # KOORDINATENANPASSUNG: axis-swap (FlingBot right-to-left ->
            # RobotSim bottom-to-top), then scale into robot camera pixels
            # ---------------------------------------------
            ...
            # Verbose coordinate-debugging prints from the rotation/scale
            # debugging phase — no longer needed day-to-day, uncomment if
            # grasp point drift shows up again.
            # print("\n===== SCALE DEBUG =====")
            ...
            # Sanity check: should be identical to payload["p1_px"]/["p2_px"]
            # via p1_robot.tolist() — kept active as a cheap consistency check.
            ...
            #
            # ATTENTION / DECISION NEEDED: the try/except/finally guard below is
            # currently DISABLED. Per project notes, this guard (buffered ack
            # read + `finally: event.set()`) exists specifically to prevent a
            # permanent deadlock if this thread dies on a malformed/split TCP
            # read. As written now, an exception in this thread will NOT set
            # robot_finished_event, and the next action will hang forever on
            # self.robot_finished_event.wait() above. Left untouched here on
            # purpose — restoring it changes runtime behavior, which is beyond
            # a cleanup pass. Recommend re-enabling unless there's a specific
            # reason it was turned off (e.g. wanting failures to surface loudly
...
with torch.no_grad():
    ...
    # prev_obs = env.transformed_obs.clone()  # only used by the disabled diff check below
    value_maps = policy.act([env.transformed_obs])[0]
    # diff = torch.mean(torch.abs(env.transformed_obs - prev_obs))
...
for action_step in range(MAX_ACTIONS):
    ...
    prev_obs = env.transformed_obs.clone()
    obs, info = env.step(value_maps)

    # Sanity check: confirms the observation actually changed after the
    # action (catches a silently no-op step). Kept active — cheap and useful.
